...
ax1.set_ylim(0, 1)
# ax1.set_ylabel("Conditional CDF", fontsize = 12)
ax1.set_ylabel(r"$F_x(\log v | h)$", fontsize = 12)

for i, cdf in enumerate(cdf_given_h):
    ax1.plot(heights, cdf, color = colours[i])
    for j in range(int(len(y)/2)):
        ax1.scatter(y[2*j + 1], gk_cdf_given_h[j][i], color = colours[i], s = 15)
for j in range(int(len(y)/2)):
    ax1.vlines(x = y[2 * j + 1], ymin = 0, ymax = gk_cdf_given_h[j][-1], color = "black", 
...
ax2.set_ylim(0, 0.45)
ax2.plot(heights, pdf_h, color = "black")
for j in range(int(len(y)/2)):
    ax2.scatter(y[2*j + 1], gk_pdf_h[j], color = "black", s = 15)
    ax2.vlines(x = y[2 * j + 1], ymin = 0, ymax = gk_pdf_h[j], color = "black", 
               linestyle = "--", alpha = .3, zorder = 0)
ax2.set_xlabel(r"Height $h$ (km asl)", fontsize = 12)
ax2.set_xticks(np.arange(h_lower, h_upper+1, 1))
ax2.set_ylabel(r"$p_H(h)$", fontsize = 12)
...
for i in reversed(range(len(thresholds))):
    ...
    for j in range(int(len(y)/2)):
        ...
        # if j > 0:
        #     ax3.plot([y[j-1], y[j]], [gk_integrand[j-1][0], gk_integrand[j][0]], color = "blue", 
        #             linestyle = "--")
...
ax1.text(9.3, 1, "a", fontweight = "bold", fontsize = 14)
ax2.text(9.3, .45, "b", fontweight = "bold", fontsize = 14)
...
plt.show()
# remove space between x-axis and bottom of the plot
# ax3.margins(y = 0)
